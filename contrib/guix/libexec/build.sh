...
(
    ...
    # First-pass guix-CMake port only supports linux HOSTS. Windows + macOS
    # need additional in-tree infrastructure that has not been ported yet:
    #   - cmake/module/Maintenance.cmake (add_windows_deploy_target /
    #     add_macos_deploy_target / setup_split_debug_script)
    #   - contrib/guix/security-check.py + symbol-check.py
    # When that infrastructure lands, restore the upstream *mingw* and
    # *darwin* branches (deploy target, codesigning tarballs) and re-add
    # those HOSTS to the matrix.
    case "$HOST" in
        *linux*) ;;
        *)
            echo "guix HOST '${HOST}' is not yet supported by this build.sh." >&2
            echo "Only linux HOSTS are wired up while the deploy + binary-check" >&2
            echo "infrastructure is ported. See feat/guix-cmake-port follow-up." >&2
            exit 1
            ;;
    esac
    ...
    (
        ...
        # Deterministically produce the binary tarball ready for release.
        # No -debug.tar.gz here yet (see split-debug TODO above).
