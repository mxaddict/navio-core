...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s navio-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\n"
                        ...
                        "By default navio-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n"
                        "To change the target wallet, use the -datadir, -wallet and -regtest/-blsctregtest/-signet/-testnet arguments.\n\n"
                        "Usage:\n"
                        "  navio-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
...
MAIN_FUNCTION
{
    ...
    if (!command) {
        tfm::format(std::cerr, "No method provided. Run `navio-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
